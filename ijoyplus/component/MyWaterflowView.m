...
#define TOP_HEADER_HEIGHT 140
#define LOADINGVIEW_HEIGHT 44
#define REFRESHINGVIEW_HEIGHT 88

@interface MyWaterflowView() <EGORefreshTableHeaderDelegate>{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.loadFooterView = [[LoadingMoreFooterView alloc]initWithFrame:CGRectMake(0, 0 + TOP_HEADER_HEIGHT, self.frame.size.width, 44.f)];
        self.loadingmore = NO;
...
- (void)setFlowdatasource:(id<MyWaterflowViewDatasource>)flowdatasource
{
    _flowdatasource = flowdatasource;
    [self initialize];
}

- (void)setFlowdelegate:(id<MyWaterflowViewDelegate>)flowdelegate
{
...
{    
    ...
    self.contentSize = CGSizeMake(self.frame.size.width, scrollHeight + LOADINGVIEW_HEIGHT + TOP_HEADER_HEIGHT);
    
    self.loadFooterView.frame = CGRectMake(0, scrollHeight + TOP_HEADER_HEIGHT, self.frame.size.width, LOADINGVIEW_HEIGHT);
...
- (void)pageScroll
{
    ...
    for (int i = 0 ; i< numberOfColumns; i++)
    {
        ...
        if ([self.visibleCells objectAtIndex:i] == nil || ((NSArray*)[self.visibleCells objectAtIndex:i]).count == 0) //everytime reloadData is called and no cells in visibleCellArray
        {
			...
			cell.frame = CGRectMake(origin_x, origin_y + TOP_HEADER_HEIGHT, width, height);
        ...
        while ( cell && ((cell.frame.origin.y - self.contentOffset.y - TOP_HEADER_HEIGHT) > 0.0001))
        {
            float origin_y = 0;
			float height = 0;
...
//        NSLog(@"%f", cell.frame.origin.y + cell.frame.size.height  - self.contentOffset.y);
        ...
        while (cell &&  ((cell.frame.origin.y + cell.frame.size.height - self.frame.size.height - self.contentOffset.y) <  0.0001))
		{
            ...
            if(rowToDisplay == [[self.cellHeight objectAtIndex:i] count] - 1 || rowToDisplay == 0)
			{
				origin_y = 0;
				cell = nil;
				break;;
