...
- (void)layoutSubviews
{
    ...
    // the thumb
    CGFloat thumbHeight = _trackImageViewNormal.bounds.size.height;   // thumb height is relative to track height
    CGFloat thumbWidth = 12; // thumb width and height keep the same ratio as the original image size
...
- (void)setStepped:(BOOL)stepped
{
    ...
    if (_stepped) {
        trackImageNormal = @"slider_bg_image";
        trackImageHighlighted = @"slider_bg_image";
    }
    else {
        trackImageNormal = @"slider_bg_image";
        trackImageHighlighted = @"slider_bg_image";
...
- (void)setValue:(float)value
{
    ...
    if(value == 0) {
        _thumbImageView.center = CGPointMake([self xForValue:value] + 6, _thumbImageView.center.y);
    } else {
...
- (void)commonInit
{
    ...
    _trackImageViewNormal = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"slider_bg_image"]];
    [self addSubview:_trackImageViewNormal];
    _trackImageViewHighlighted = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"slider_bg_image"]];
    [self addSubview:_trackImageViewHighlighted];
    
    // thumb knob
    _thumbImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"slider_thumb"]];
