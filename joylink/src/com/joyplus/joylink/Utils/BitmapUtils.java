...
public class BitmapUtils {
    ...
    public static int computeSampleSize(int width, int height,
        ...
        return initialSize <= 8
                ? JoylinkUtils.nextPowerOf2(initialSize)
                : (initialSize + 7) / 8 * 8;
    }
    ...
    public static int computeSampleSizeLarger(int w, int h,
        ...
        return initialSize <= 8
                ? JoylinkUtils.prevPowerOf2(initialSize)
                : initialSize / 8 * 8;
    }
    ...
    public static int computeSampleSize(float scale) {
        JoylinkUtils.assertTrue(scale > 0);
    ...
    public static String createVideoThumbnailtoSD(String filePath) {
    	File cacheDir = new File(Constant.PATH);
    	File mFile = JoylinkUtils.getCacheFile(cacheDir,filePath);
    	Bitmap bitmap = createVideoThumbnail(filePath);
		if (bitmap != null){
			 try {
				 FileOutputStream out=new FileOutputStream(mFile,false);//不采用追加方式
				 bitmap.compress(Bitmap.CompressFormat.JPEG,
		                COMPRESS_JPEG_QUALITY, out);
				return JoylinkUtils.getCacheFileName(filePath);
			 } catch (Exception e) {
			       e.printStackTrace();
			}
