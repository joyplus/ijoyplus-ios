...
- (void)viewDidLoad {
    ...
    self.messageTextField.frame = CGRectMake(self.messageTextField.frame.origin.x, self.messageTextField.frame.origin.y, 400, self.messageTextField.frame.size.height);
...
{
    if (error == nil) {
        NSString *temMsg = @"亲，说说您的使用感受吧。如果遇到哪部片子有问题，请附带片名，我们会在第一时间解决哦。您的陪伴会让我们做的更好，谢谢支持！";
...
- (UIView *)bubbleView:(NSString *)text from:(BOOL)fromSelf {
    ...
    if(fromSelf){
        ...
        [headImageView setImageWithURL:[NSURL URLWithString:avatarUrl] placeholderImage:[UIImage imageNamed:@"self_icon"]];
        ...
        cellView.frame = CGRectMake(445.0f-bubbleImageView.frame.size.width, 0.0f,bubbleImageView.frame.size.width+50.0f, bubbleImageView.frame.size.height+30.0f);
...
-(void) autoMovekeyBoard: (float) h{
	...
	toolbar.frame = CGRectMake(0.0f, (float)(self.navigationController.view.frame.size.height-h-88), self.navigationController.view.frame.size.width, 44.0f);
